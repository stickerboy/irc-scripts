...
msg.reply "Logs are not currently being tracked"
  ...
  def log_html_message(msg)
    ...
    str = <<-HTML
    <div id="#{anchor}">
        <div class="msgtime"><a href="##{anchor}">#{msg.time.strftime(@timelogformat)}</a></div>
        <div class="msgnick #{determine_status(msg)}">#{msg.user}</div>
        <div class="msgmessage">#{converter.convert(CGI.escape_html(msg.message))}</div>
    </div>
    HTML
  ...
  def log_own_htmlmessage(text, is_notice)
    ...
    <div id="#{anchor}">
        <div class="msgtime"><a href="##{anchor}">#{time.strftime(@timelogformat)}</a></div>
        <div class="msgnick selfbot">#{bot.nick}</div>
        <div class="msgmessage">#{converter.convert(CGI.escape_html(text))}</div>
    </div>
    HTML
  end
  ...
  def log_html_action(msg)
    ...
    <div id="#{anchor}">
        <div class="msgtime"><a href="##{anchor}">#{msg.time.strftime(@timelogformat)}</a></div>
        <div class="msgnick">*</div>
        <div class="msgaction"><span class="actionnick #{determine_status(msg)}">#{msg.user.name}</span>&nbsp;#{converter.convert(CGI.escape_html(msg.action_message))}</div>
    </div>
    HTML
  ...
  def log_html_topic(msg)
    ...
    <div id="#{anchor}">
        <div class="msgtime"><a href="##{anchor}">#{msg.time.strftime(@timelogformat)}</a></div>
        <div class="msgnick">*</div>
        <div class="msgtopic"><span class="actionnick #{determine_status(msg)}">#{msg.user.name}</span>&nbsp;changed the topic to “#{CGI.escape_html(msg.message)}”.</div>
    </div>
    HTML
  end
  ...
  def log_html_nick(msg)
    ...
    <div id="#{anchor}">
        <div class="msgtime"><a href="##{anchor}">#{msg.time.strftime(@timelogformat)}</a></div>
        <div class="msgnick">--</div>
        <div class="msgnickchange"><span class="actionnick #{determine_status(msg, oldnick)}">#{oldnick}</span>&nbsp;is now known as <span class="actionnick #{determine_status(msg, msg.message)}">#{msg.message}</span>.</div>
    </div>
    HTML
  end
  ...
  def log_html_join(msg)
    ...
    <div id="#{anchor}">
        <div class="msgtime"><a href="##{anchor}">#{msg.time.strftime(@timelogformat)}</a></div>
        <div class="msgnick">--&gt;</div>
        <div class="msgjoin"><span class="actionnick #{determine_status(msg)}">#{msg.user.name}</span>&nbsp;entered #{msg.channel.name}.</div>
    </div>
    HTML
  end
  ...
  def log_html_leaving(msg, leaving_user)
    ...
    <div id="#{anchor}">
        <div class="msgtime"><a href="##{anchor}">#{msg.time.strftime(@timelogformat)}</a></div>
        <div class="msgnick">&lt;--</div>
        <div class="msgleave"><span class="actionnick #{determine_status(msg)}">#{leaving_user.name}</span>&nbsp;#{text}.</div>
    </div>
    HTML
  end
  ...
  def log_html_modechange(msg, changes)
    ...
    @htmllogfile.write(<<-HTML)
        ...
        <div class="msgtime"><a href="##{anchor}">#{msg.time.strftime(@timelogformat)}</a></div>
        <div class="msgnick">--</div>
        <div class="msgmode">Mode #{change} by <span class="actionnick #{determine_status(msg)}">#{msg.user.name}</span>.</div>
    </div>
    HTML
  end
...
#{@extrahead}
  ...
  <body>
    ...
    unless bot.channels.empty?
      @htmllogfile.puts <<-HTML
        <div>
            <div class="msgtime">#{Time.now.strftime(@timelogformat)}</div>
            <div class="msgnick">(system message)</div>
            <div class="msgtopic">The topic for this channel is currently “#{CGI.escape_html(bot.channels.first.topic)}”.</div>
        </div>
      HTML
    end
  end
