...
Irssi::settings_add_str('misc','countdown_target','2015 04 06 01 00 00');
Irssi::settings_add_str('misc','countdown_message','#HUNTtheTRUTH - Next audio log release:');
Irssi::settings_add_str('misc','countdown_command','!countdown');
Irssi::settings_add_str('misc','countdown_chan','#Halo5');
...
sub sig_public {
	...
	if ($msg eq $ccmd and lc($target) eq lc($cchan))
	{
		if ($ctarget =~ /^(\d+?) (\d+?) (\d+?) (\d+?) (\d+?) (\d+?)$/)
		{
			...
			$server->command("msg $target $cinfo - http://huntthetruth.tumblr.com");
		} else
		{
